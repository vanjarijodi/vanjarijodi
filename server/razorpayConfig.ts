...
// Default initial config with environment variable fallbacks (Production LIVE Mode)
let runtimeConfig: RazorpayServerConfig = {
  enabled: true,
  env: (process.env.RAZORPAY_ENV === 'TEST' ? 'TEST' : 'LIVE') as 'TEST' | 'LIVE',
  keyId: process.env.RAZORPAY_KEY_ID || 'rzp_live_VanjariJodiPay',
...
export function getRazorpayServerConfig(): RazorpayServerConfig {
  return {
    ...runtimeConfig,
    keyId: process.env.RAZORPAY_KEY_ID || runtimeConfig.keyId,
    secretKey: process.env.RAZORPAY_KEY_SECRET || runtimeConfig.secretKey,
    webhookSecret: process.env.RAZORPAY_WEBHOOK_SECRET || runtimeConfig.webhookSecret,
    env: (process.env.RAZORPAY_ENV || runtimeConfig.env) as 'TEST' | 'LIVE',
  };
}
