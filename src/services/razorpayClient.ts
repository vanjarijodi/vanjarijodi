...
export async function initiateRazorpayPayment(options: RazorpayCheckoutOptions): Promise<void> {
  ...
  try {
    ...
    const resolvedOrderId = orderData.order_id || orderData.orderId || orderData.id;
    const resolvedAmount = orderData.amount; // in paise
    const resolvedKey = orderData.key_id || orderData.keyId || config.keyId;
    ...
    const rzpOptions = {
      ...
      handler: async (response: {
        ...
        try {
          ...
          if (verifyRes.ok && (verifyData.success || verifyData.status === 'ok')) {
            onSuccess(verifyData);
          } else {
            onError(verifyData.message || verifyData.error || 'पेमेंट स्वाक्षरी पडताळणी अयशस्वी झाली. कृपया ग्राहक सेवेशी संपर्क साधा.');
          }
        } catch (vErr: any) {
