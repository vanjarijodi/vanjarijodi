...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginAsGuest = (mobile?: string, name?: string, location?: string) => {
    ...
    if (rawClean && rawClean.length >= 10) {
      const blockedProfile = profiles.find((p) => {
        const pClean = (p.mobile || '').replace(/\D/g, '');
        return p.isBlocked && pClean.includes(rawClean.slice(-10));
      });

      if (blockedProfile) {
        alert(
          language === 'mr'
            ? '🚫 क्षमस्व! या मोबाईल क्रमांकाशी संलग्न खाते ॲडमिनद्वारे ब्लॉक करण्यात आले आहे. गेस्ट म्हणूनही प्रवेश करता येणार नाही.'
            : 'Sorry! The account associated with this mobile number has been blocked by Admin.'
        );
        return;
      }
    }
    ...
    const guestUser: UserProfile = {
      ...
      email: siteConfig?.contactEmail || 'gitevijay123@gmail.com',
