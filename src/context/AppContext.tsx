...
interface AppContextType {
  ...
  markProfileAsViewed: (profileId: string) => void;
  ...
  addNotification: (notification: Partial<NotificationItem> & { title: string; message: string; type?: string }) => void;
  ...
  // Truecaller & Mobile Number Verification
  isPhoneAuthModalOpen: boolean;
  setIsPhoneAuthModalOpen: (open: boolean) => void;
  verifyPhoneNumber: (profileId: string, method: 'truecaller' | 'otp' | 'admin', truecallerName?: string) => Promise<boolean>;
  togglePhoneVerified: (profileId: string, status?: boolean) => void;
  ...
  // Kundali Matching (Ashtakoot Gun Milan) Modal State
  isKundaliModalOpen: boolean;
  setIsKundaliModalOpen: (open: boolean) => void;
  selectedKundaliCandidate: UserProfile | null;
  setSelectedKundaliCandidate: (profile: UserProfile | null) => void;
  openKundaliMatching: (candidate?: UserProfile | null) => void;

  // Single Kundli / Birth Chart Report Modal State
  isSingleKundliModalOpen: boolean;
  setIsSingleKundliModalOpen: (open: boolean) => void;
  openSingleKundliGenerator: () => void;

  // GitHub Code & Media Sync Center Modal State
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Deleted Profile IDs Blacklist tracking
  const [deletedProfileIds, setDeletedProfileIds] = useState<string[]>(() => {
    return safeJsonParse<string[]>('vanjari_jodi_deleted_profile_ids', []);
  });

  useEffect(() => {
    try {
      localStorage.setItem('vanjari_jodi_deleted_profile_ids', JSON.stringify(deletedProfileIds));
    } catch (e) {}
  }, [deletedProfileIds]);
  ...
  const addDeletedProfileId = (id: string) => {
    ...
    setDeletedProfileIds((prev) => {
      ...
      try {
        localStorage.setItem('vanjari_jodi_deleted_profile_ids', JSON.stringify(updated));
      } catch (e) {}
      return updated;
    });
  };

  const removeDeletedProfileId = (id: string) => {
    const cleanId = String(id || '').trim();
    setDeletedProfileIds((prev) => {
      const updated = prev.filter((x) => x !== cleanId);
      try {
        localStorage.setItem('vanjari_jodi_deleted_profile_ids', JSON.stringify(updated));
      } catch (e) {}
      return updated;
  ...
  const [profiles, setProfiles] = useState<UserProfile[]>(() => {
    ...
    try {
      const savedDeleted = localStorage.getItem('vanjari_jodi_deleted_profile_ids');
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Filter out legacy dummy placeholder bride profiles (vj-bride-*) and deleted profiles
          const cleaned = parsed.filter(
            (p: UserProfile) => p && p.id && !p.id.startsWith('vj-bride-') && !deletedIds.includes(String(p.id).trim()) && !(p as any).isDeleted
          );
          return cleaned.length > 0 ? cleaned : INITIAL_PROFILES.filter(p => !deletedIds.includes(String(p.id).trim()));
        }
        return INITIAL_PROFILES.filter(p => !deletedIds.includes(String(p.id).trim()));
      } catch (e) {
        return INITIAL_PROFILES.filter(p => !deletedIds.includes(String(p.id).trim()));
      }
    }
  ...
  useEffect(() => {
    const unsubProfiles = listenToProfiles((firestoreProfiles) => {
      ...
      try {
        const raw = localStorage.getItem('vanjari_jodi_deleted_profile_ids');
        if (raw) currentDeleted = JSON.parse(raw);
      } catch (e) {}

      if (firestoreProfiles && Array.isArray(firestoreProfiles)) {
        const filtered = firestoreProfiles.filter(
          (p) => p && p.id && !currentDeleted.includes(String(p.id).trim()) && !(p as any).isDeleted
        );
    ...
    const unsubConfig = listenToSiteConfig((remoteConfig) => {
      if (remoteConfig) {
        const sanitizeLogo = (url?: string) => {
          if (url && (url.startsWith('data:image/') || url.includes('vanjari-jodi') || url.includes('logo'))) {
            return url;
          }
          return `/vanjari-jodi-official-logo.png?v=${Date.now()}`;
        };
        const sanitizeBhagwangad = (url?: string) => {
          if (!url || url.includes('wikimedia') || url.includes('http://') || url.includes('https://')) {
            return '/vanjari-jodi-official-logo.png';
          }
          return url;
        };
    ...
    const unsubPlans = listenToPlans((remotePlans) => {
      if (remotePlans && Array.isArray(remotePlans) && remotePlans.length > 0) {
        setPlansList(remotePlans);
        try {
          localStorage.setItem('vanjari_jodi_plans', JSON.stringify(remotePlans));
        } catch (e) {}
      }
    });
  ...
  const [currentUser, setCurrentUser] = useState<UserProfile | null>(() => {
    ...
    if (savedUser) {
      try {
        ...
        if (parsed && parsed.id) {
          // GUEST LOGIN IS PERMANENTLY DISABLED: Immediately purge any guest session!
          if (parsed.isGuest || parsed.id.startsWith('guest-') || parsed.id.startsWith('guest_') || (parsed.fullName && parsed.fullName.includes('Guest')) || (parsed.fullName && parsed.fullName.includes('पाहुणे'))) {
            localStorage.removeItem('vanjari_jodi_current_user');
            return null;
          }
          // If the profile was blocked by Admin, do not restore session
          if (parsed.isBlocked) {
            localStorage.removeItem('vanjari_jodi_current_user');
            return null;
          }
  ...
  useEffect(() => {
    if (currentUser) {
      if (currentUser.isBlocked || currentUser.isGuest || currentUser.id.startsWith('guest-') || currentUser.id.startsWith('guest_')) {
        setCurrentUser(null);
        localStorage.removeItem('vanjari_jodi_current_user');
        return;
      }
    ...
    if (currentUser && profiles.length > 0) {
      ...
      if (updatedProfile) {
        if (updatedProfile.isBlocked) {
          // Admin blocked this user in real-time -> Force logout immediately!
          setCurrentUser(null);
          localStorage.removeItem('vanjari_jodi_current_user');
          alert(language === 'mr' ? '🚫 ॲडमिनने तुमचे खाते ब्लॉक केले आहे. तुमचे लॉगिन बंद करण्यात आले आहे.' : 'Your account has been blocked by Admin.');
        } else if (JSON.stringify(updatedProfile) !== JSON.stringify(currentUser)) {
    ...
    if (saved) {
      try {
        ...
        return {
          ...INITIAL_SITE_CONFIG,
          ...parsed,
          logoUrl: (parsed.logoUrl && (parsed.logoUrl.startsWith('data:image/') || parsed.logoUrl.includes('vanjari-jodi') || parsed.logoUrl.includes('logo'))) ? parsed.logoUrl : `/vanjari-jodi-official-logo.png?v=${Date.now()}`,
          bhagwangadImg: (parsed.bhagwangadImg && !parsed.bhagwangadImg.includes('wikimedia') && !parsed.bhagwangadImg.includes('http')) ? parsed.bhagwangadImg : '/vanjari-jodi-official-logo.png',
          adminCredentials: {
  ...
  // Track viewed profiles to prevent immediate repeat viewing while looping feed smoothly
  const [viewedProfileIds, setViewedProfileIds] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('vanjari_jodi_viewed_profile_ids');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('vanjari_jodi_viewed_profile_ids', JSON.stringify(viewedProfileIds));
    } catch (e) {
      console.warn('Failed to save viewedProfileIds:', e);
    }
  }, [viewedProfileIds]);
  ...
  const resetViewedProfiles = () => {
    setViewedProfileIds([]);
    try {
      localStorage.removeItem('vanjari_jodi_viewed_profile_ids');
    } catch (e) {}
  };

  // Smart feed ordering: Unviewed profiles appear FIRST. Once all profiles are viewed, feed seamlessly loops back.
  const sortedFilteredProfiles = useMemo(() => {
    const unviewed = filteredProfiles.filter((p) => !viewedProfileIds.includes(p.id));
    const viewed = filteredProfiles.filter((p) => viewedProfileIds.includes(p.id));

    if (unviewed.length > 0) {
      return [...unviewed, ...viewed];
    }
    return filteredProfiles;
  }, [filteredProfiles, viewedProfileIds]);

  // 5. Shortlisting
  const [shortlistedIds, setShortlistedIds] = useState<string[]>(() => {
    return safeJsonParse<string[]>('vanjari_jodi_shortlists', []);
  });
  ...
  const [interests, setInterests] = useState<Interest[]>(() => {
    return safeJsonParse<Interest[]>('vanjari_jodi_interests', []);
  });
  ...
  const sendInterest = (toUserId: string) => {
    ...
    const isFestiveFree = Boolean(siteConfig?.isFestiveFreeModeEnabled);
    const isUserPaid = isFestiveFree || (currentUser.membership && currentUser.membership !== 'free' && !isProfilePlanExpired(currentUser));

    if (targetUser && autoApprove) {
      if (targetLikesMe && isMutualUnlockEnabled) {
        if (isUserPaid || isFestiveFree) {
          setUnlockedContacts((prev) => (prev.includes(toUserId) ? prev : [...prev, toUserId]));
          addNotification({
            userId: toUserId,
            title: '🎉 Mutual Like Match! Contact Unlocked',
            titleMr: isFestiveFree ? '🎉 सण-उत्सव मोफत ऑफर: म्युचुअल मॅच! नंबर अनलॉक!' : '🎉 म्युचुअल मॅच! मोबाईल नंबर अनलॉक झाला!',
          ...
          alert(`🎉 म्युचुअल मॅच (Mutual Match)! ${targetUser.fullName || 'सदस्याने'} सुद्धा तुम्हाला आधीच लाईक केले होते.${isFestiveFree ? '\n\n🎊 सण-उत्सव मोफत ऑफर सक्रिय असल्याने तुम्हा दोघांचे मोबाईल नंबर मोफत अनलॉक झाले आहेत!' : '\n\nएकमेकांनी लाईक केल्यामुळे तुम्हा दोघांचे मोबाईल नंबर आता अनलॉक झाले आहेत!'}`);
        } else {
          addNotification({
            userId: toUserId,
  ...
  // Real-time Firestore sync for membership plans
  useEffect(() => {
    const unsubscribe = listenToPlans((remotePlans) => {
      if (remotePlans && Array.isArray(remotePlans) && remotePlans.length > 0) {
        setPlansList((prev) => {
          let hasStale399 = false;
          const merged = remotePlans.map((rp) => {
            if (rp.id === 'welcome_offer') {
              if (rp.price === 399 || !rp.price || rp.price === 0) {
                hasStale399 = true;
              }
              return {
                ...rp,
                price: rp.price === 399 || !rp.price ? 398 : rp.price,
                nameMr: rp.nameMr?.replace('३९९', '३९८')?.replace('399', '398') || 'वेलकम स्पेशल ऑफर प्लॅन - ₹३९८ (६ महिने)',
                badgeText: rp.badgeText?.replace('३९९', '३९८')?.replace('399', '398') || '🔥 ५०% सवलत - वेलकम ऑफर (रु. ३९८/-)'
              };
            }
            return rp;
          });
          try {
            localStorage.setItem('vanjari_jodi_plans', JSON.stringify(merged));
            if (hasStale399) {
              savePlansToFirestore(merged);
            }
          } catch (e) {}
          return merged;
        });
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>(() => {
    return safeJsonParse<ChatMessage[]>('vanjari_jodi_chats', []);
  });
  ...
  const [likedProfileIds, setLikedProfileIds] = useState<string[]>(() => {
    return safeJsonParse<string[]>('vanjari_jodi_liked_profiles', []);
  });

  const [pendingLikes, setPendingLikes] = useState<any[]>(() => {
    return safeJsonParse<any[]>('vanjari_jodi_pending_likes', []);
  });
  ...
  const checkCanUserChatWithMember = (
    ...
    if (!currentUser) {
      return {
        ...
        isPaidPlan: false,
        targetUser: null,
        reason: 'कृपया चॅट करण्यासाठी प्रथम लॉगिन करा.'
      };
    }
    ...
    if (!targetUser) {
      return {
        ...
        isPaidPlan: false,
        targetUser: null,
        reason: 'सदस्य प्रोफाईल सापडले नाही.'
      };
    }

    // 1. Mutual Like Check (User A liked User B AND User B liked User A)
    const iLikedTarget =
      likedProfileIds.includes(targetUserId) ||
      (currentUser.shortlistedProfiles || []).includes(targetUserId) ||
      (targetUser.likedByUsers || []).includes(currentUser.id) ||
      interests.some(
        (int) => int.fromUserId === currentUser.id && int.toUserId === targetUserId && int.status !== 'rejected'
      );

    const targetLikedMe =
      (currentUser.likedByUsers || []).includes(targetUserId) ||
      (targetUser.shortlistedProfiles || []).includes(currentUser.id) ||
      (targetUser.likedProfileIds || []).includes(currentUser.id) ||
      interests.some(
        (int) => int.fromUserId === targetUserId && int.toUserId === currentUser.id && int.status !== 'rejected'
      );

    const isMutualLiked = Boolean(iLikedTarget && targetLikedMe);

    // 2. Truecaller / Phone Verified Check
    const isPhoneVerified = Boolean(
      currentUser.isPhoneVerified ||
      currentUser.truecallerVerified ||
      currentUser.phoneVerificationMethod === 'truecaller' ||
      currentUser.phoneVerificationMethod === 'otp' ||
      currentUser.phoneVerificationMethod === 'admin'
    );

    // 3. Paid Plan Check (Or Offer Mode)
    const isOfferActive = Boolean(siteConfig?.isOfferModeEnabled || siteConfig?.disableAllPaymentsInOfferMode);
    const isPaidPlan = Boolean(
      isOfferActive ||
      (currentUser.membership && currentUser.membership !== 'free' && !isProfilePlanExpired(currentUser))
    );

    const allowed = Boolean(isMutualLiked && isPhoneVerified && isPaidPlan);

    let reason = '';
    if (!isMutualLiked) {
      reason = 'सदस्यांशी चॅट करण्यासाठी दोघांनी एकमेकांना लाईक केलेले (Mutual Match) असणे आवश्यक आहे.';
    } else if (!isPhoneVerified) {
      reason = 'चॅट करण्यासाठी आपला मोबाईल नंबर Truecaller द्वारे प्रमाणित (Verified) असणे आवश्यक आहे.';
    } else if (!isPaidPlan) {
      reason = 'थेट चॅट करण्यासाठी आपले पेमेंट/सबस्क्रिप्शन सक्रिय असणे आवश्यक आहे.';
    }
  ...
  const sendChatMessage = (
    ...
    // Enforce Rule: Member-to-Member chat requires Mutual Like + Truecaller Verification + Paid Plan
    if (!isAdminLoggedIn && !currentUser.isAdmin && receiverId !== 'admin') {
      const chatPerm = checkCanUserChatWithMember(receiverId);
      if (!chatPerm.allowed) {
        return {
          success: false,
          message: chatPerm.reason || 'सदस्यांशी थेट चॅट करण्यासाठी म्युच्युअल लाईक, Truecaller पडताळणी व पेड प्लॅन आवश्यक आहे.'
        };
      }
    }
    ...
    const target = profiles.find((p) => p.id === receiverId);
    trackUserAction(
      'चॅट संदेश पाठवला',
      `${currentUser.fullName} ने ${target?.fullName || receiverId} यांना चॅट संदेश पाठवला: "${text.slice(0, 35)}..."`,
      'chat',
      receiverId,
      target?.fullName
    );
  ...
  const addNotification = (item: Partial<NotificationItem> & { title: string; message: string; type?: string }) => {
    const newNotif: NotificationItem = {
      userId: item.userId || currentUser?.id || 'admin',
      title: item.title,
      titleMr: item.titleMr || item.title,
      message: item.message,
      messageMr: item.messageMr || item.message,
    ...
    if (isTargetedToCurrentSession) {
      const shouldPlaySound = siteConfig?.enableSoundNotifications !== false;
  ...
  const approvePaymentRequest = (id: string) => {
    ...
    // Add notification & trigger direct Push Notification to user's device
    addNotification({
      userId: target.userId,
      title: 'Payment Approved - VIP Membership Active! 👑',
      titleMr: '🎉 तुमचे पेमेंट मंजूर झाले व VIP सदस्यत्व सक्रिय झाले! 👑',
      message: `Your payment for ${target.planName} has been verified and activated. You can now access all contact numbers & biodatas.`,
      messageMr: `तुमचे ${target.planName} पेमेंट मंजूर झाले असून प्रीमियम सेवा सक्रिय झाली आहे. आता तुम्ही सर्व वधू-वर संपर्क नंबर व बायोडाटा पाहू शकता!`,
      type: 'approval',
    });
    ...
    if (typeof sendPushNotification === 'function') {
      sendPushNotification(
        target.userId,
        '🎉 VIP सदस्यत्व सक्रिय झाले! (वंजारी जोडी)',
        `तुमचे ${target.planName} पेमेंट मंजूर झाले असून सर्व संपर्क नंबर अनलॉक झाले आहेत. शुभेच्छा!`
      );
    }
  };
  ...
  const [heroSlides, setHeroSlides] = useState<HeroSlide[]>(() => {
    return safeJsonParse<HeroSlide[]>('vanjari_jodi_hero_slides', INITIAL_HERO_SLIDES);
  });
  useEffect(() => {
  ...
  const [counters, setCounters] = useState<CounterItem[]>(() => {
    return safeJsonParse<CounterItem[]>('vanjari_jodi_counters', INITIAL_COUNTERS);
  });
  useEffect(() => {
  ...
  const [contactRequests, setContactRequests] = useState<ContactRequest[]>(() => {
    const parsed = safeJsonParse<any[]>('vanjari_jodi_contact_requests', []);
    return Array.isArray(parsed) ? parsed.filter((r: any) => !r.requesterId?.startsWith('vj-1') && !r.targetProfileId?.startsWith('vj-1')) : [];
  });
  ...
  const isContactAuthorizedForUser = (targetProfileId: string): boolean => {
    ...
    // Strict Unapproved User Block: If user is logged in but not yet approved by Admin, block contact view
    if (currentUser && currentUser.isApproved === false && !currentUser.isAdmin) {
      return false;
    }
    ...
    if (currentUser && siteConfig?.enableMutualLikeContactUnlock !== false) {
      ...
      if (iLikeTarget && targetLikesMe) {
        // जर Truecaller पडताळणीची अट सुरू असेल
        const requireTruecaller = siteConfig?.requireTruecallerForMutualUnlock !== false;
        const isCurrentUserVerified = currentUser.isPhoneVerified || currentUser.truecallerVerified;
        
        if (requireTruecaller && !isCurrentUserVerified) {
          // युझरने स्वतःचा नंबर Truecaller द्वारे व्हेरिफाय केलेला नाही, त्यामुळे अनलॉक होणार नाही
          return false;
        }

        // Mutual match exists & Truecaller check passed!
        return true;
      }

      // If mode is strictly mutual like only, require mutual match
      if (siteConfig?.contactUnlockMode === 'mutual_like_only' || siteConfig?.requireMutualLikeAndVerificationToView === true) {
        return false;
      }
    }
  ...
  const logout = useCallback(() => {
    ...
    try {
      ...
      localStorage.removeItem('vanjari_jodi_auth_token');
    ...
    setIsUserSecurityOpen(false);
    setIsAppShareOpen(false);
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('vanjari_new_notification_toast', {
          detail: {
            title: 'लॉगआउट यशस्वी',
            body: 'आपण यशस्वीरित्या खात्यातून बाहेर पडला आहात.',
          },
        })
      );
    } catch (e) {}
  ...
  const rejectProfile = (profileId: string) => {
    ...
    if (!cleanId) return;
    addDeletedProfileId(cleanId);
    setProfiles((prev) => prev.filter((p) => String(p.id).trim() !== cleanId));
    deleteDocFromFirestore('profiles', cleanId);
    try {
      fetch(`/api/profiles/${encodeURIComponent(cleanId)}`, { method: 'DELETE' }).catch(() => {});
    } catch (e) {}
  };
      ...
      prev.map((p) => {
        if (p.id === profileId) {
          const updated: UserProfile = {
            ...
            kundliCredits: (p.kundliCredits || 0) + (tier === 'single_kundli' ? 1 : 0),
          };
          syncDocToFirestore('profiles', updated.id, updated);
          return updated;
        }
      ...
      setCurrentUser((prev) =>
        prev
          ? {
              ...
              kundliCredits: (prev.kundliCredits || 0) + (tier === 'single_kundli' ? 1 : 0),
            }
          : null
      );
    }
  ...
  const addProfile = (newProfile: UserProfile) => {
    ...
    // Auto Push Notification Trigger on New Biodata
    if (siteConfig?.enableAutoPushNewBiodata !== false) {
      addBroadcastNotification(
        '✨ नवीन वंजारी बायोडाटा जोडला गेला!',
        `${profileToSave.fullName} (${profileToSave.gender === 'bride' ? 'वधू' : 'वर'}, ${profileToSave.age || ''} वर्षे, ${profileToSave.district || 'महाराष्ट्र'}) - वंजारी जोडीवर नवीन बायोडाटा जोडला गेला आहे!`
      );
    }
  };
  ...
  const replyAdminSupportMessage = (
    ...
    addNotification({
      ...
      message: message || 'ॲडमिन यांनी तुम्हाला थेट संदेश पाठवला आहे.',
      messageMr: `${targetUser ? targetUser.fullName : 'सदस्य'}जी, ॲडमिन कडून संदेश: "${message || (detectedType === 'image' ? 'फोटो' : 'दस्तऐवज')}"`,
      type: 'chat',
    });
    ...
    if (typeof sendPushNotification === 'function') {
      sendPushNotification(
        ...
        message || 'ॲडमिन यांनी तुम्हाला थेट संदेश पाठवला आहे. वाचण्यासाठी ॲप उघडा.'
      );
    }
  };
  ...
  const softDeleteProfile = async (profileId: string) => {
    ...
    const target = profiles.find((p) => String(p.id).trim() === cleanId) ||
      (currentUser && String(currentUser.id).trim() === cleanId ? currentUser : null);

    const targetName = target?.fullName || (currentUser && String(currentUser.id).trim() === cleanId ? currentUser.fullName : 'सदस्य');

    // 3. Trash photos and push to Recycle Bin
    if (target) {
      ...
      const newItem: RecycleBinItem = {
        ...
        title: `${target.fullName} (${target.gender === 'bride' ? 'वधू' : 'वर'} - ${target.district || 'महाराष्ट्र'})`,
        deletedAt: new Date().toLocaleString('mr-IN'),
        data: { ...target, isDeleted: true },
      };

      setRecycleBin((prev) => [newItem, ...prev.filter(r => (r.data as any)?.id !== cleanId)]);
    }

    // 4. Instantly remove from local profiles state & localStorage
    setProfiles((prev) => prev.filter((p) => String(p.id).trim() !== cleanId));
    try {
      const stored = localStorage.getItem('vanjari_jodi_profiles');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed)) {
          const updated = parsed.filter((p: any) => String(p?.id).trim() !== cleanId);
          localStorage.setItem('vanjari_jodi_profiles', JSON.stringify(updated));
        }
      }
    } catch (e) {}

    // 5. Delete from Firestore & flag as isDeleted
    try {
      await syncDocToFirestore('profiles', cleanId, { isDeleted: true, isApproved: false, deletedAt: new Date().toISOString() });
      await deleteDocFromFirestore('profiles', cleanId);
      await syncDocToFirestore('deleted_profiles', cleanId, { profileId: cleanId, deletedAt: new Date().toISOString() });
    } catch (err) {
      console.warn('Firestore profile deletion error:', err);
    }

    // 6. Delete on backend server
    try {
      fetch(`/api/profiles/${encodeURIComponent(cleanId)}`, { method: 'DELETE' }).catch(() => {});
    } catch (e) {}

    // 7. If currentUser is the one being deleted, clear session completely
    if (currentUser && String(currentUser.id).trim() === cleanId) {
      setCurrentUser(null);
      try {
        localStorage.removeItem('vanjari_jodi_current_user');
        sessionStorage.removeItem('vanjari_jodi_current_user');
    ...
    logActivity('Soft Delete', `${targetName} यांची प्रोफाईल व फोटो रिसायकल बिन मध्ये टाकले`, 'Admin');
  };

  const bulkSoftDeleteProfiles = async (profileIds: string[]) => {
    if (!profileIds || profileIds.length === 0) return;
    const cleanIds = profileIds.map((id) => String(id).trim()).filter(Boolean);

    cleanIds.forEach((pid) => addDeletedProfileId(pid));

    cleanIds.forEach((pid) => {
      const target = profiles.find((p) => String(p.id).trim() === pid);
      if (target) {
        if (target.photoUrl && target.photoUrl.trim() !== '') {
          trashPhoto(target.id, target.photoUrl, 'avatar', target.fullName);
        }
        ...
        setRecycleBin((prev) => [newItem, ...prev.filter(r => (r.data as any)?.id !== pid)]);
      }
      deleteDocFromFirestore('profiles', pid);
      fetch(`/api/profiles/${encodeURIComponent(pid)}`, { method: 'DELETE' }).catch(() => {});
    });

    setProfiles((prev) => prev.filter((p) => !cleanIds.includes(String(p.id).trim())));
    ...
    try {
      ...
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed)) {
          const updated = parsed.filter((p: any) => !cleanIds.includes(String(p?.id).trim()));
          localStorage.setItem('vanjari_jodi_profiles', JSON.stringify(updated));
        }
      }
    } catch (e) {}

    logActivity('Bulk Soft Delete', `${cleanIds.length} प्रोफाईल व फोटो एकत्र रिसायकल बिनमध्ये हलवले`, 'Admin');
  };
  ...
  const restoreRecycleItem = (id: string) => {
    ...
    if (item.originalType === 'biodata') {
      const restored = { ...item.data, isDeleted: false };
      const cleanId = String(restored.id).trim();
      removeDeletedProfileId(cleanId);
      setProfiles((prev) => [restored, ...prev.filter((p) => String(p.id).trim() !== cleanId)]);
      syncDocToFirestore('profiles', cleanId, restored);
      deleteDocFromFirestore('deleted_profiles', cleanId);
  ...
  const permanentDeleteRecycleItem = (id: string) => {
    ...
    if (item) {
      if (item.data?.id) {
        const cleanId = String(item.data.id).trim();
        addDeletedProfileId(cleanId);
        deleteDocFromFirestore('profiles', cleanId);
        fetch(`/api/profiles/${encodeURIComponent(cleanId)}`, { method: 'DELETE' }).catch(() => {});
      }
  ...
  const bulkPermanentDeleteRecycleItems = (ids: string[]) => {
    const itemsToDelete = recycleBin.filter((r) => ids.includes(r.id));
  ...
  const [bioDataSubmissions, setBioDataSubmissions] = useState<BioDataSubmission[]>(() => {
    const saved = localStorage.getItem('vanjari_jodi_biodata_submissions');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
  ...
  useEffect(() => {
    localStorage.setItem('vanjari_jodi_biodata_submissions', JSON.stringify(bioDataSubmissions));
  }, [bioDataSubmissions]);

  const registerCandidateDirectly = (candidateData: CandidateRegistrationData) => {
    const newProfileId = 'vj-' + Math.floor(1000 + Math.random() * 9000);
    const newProfile: UserProfile = {
      id: newProfileId,
      fullName: candidateData.fullName || 'नवीन उमेदवार',
      gender: candidateData.gender || 'bride',
      dob: candidateData.birthDate || '1998-01-01',
      age: candidateData.birthDate ? Math.max(18, new Date().getFullYear() - new Date(candidateData.birthDate).getFullYear()) : 25,
      subCaste: 'वंजारी',
      religion: 'हिंदू',
      height: candidateData.height || "5'4\"",
      weight: '55 kg',
      bloodGroup: 'B+',
      maritalStatus: 'never_married',
      education: candidateData.education || 'पदवीधर',
      occupation: candidateData.occupation || 'नोकरी',
      income: '३ ते ५ लाख',
      city: candidateData.taluka || 'नाशिक',
      taluka: candidateData.taluka || 'नाशिक',
      district: candidateData.district || 'नाशिक',
      mobile: candidateData.mobileNumber || '9876543210',
      email: 'member' + Date.now() + '@vanjarijodi.com',
      fatherOccupation: 'शेती / व्यवसाय',
      motherOccupation: 'गृहिणी',
      brothers: 1,
      sisters: 1,
      familyType: 'एकत्र कुटुंब (Joint)',
      expectations: 'वंजारी समाजातील सुशिक्षित व सुसंस्कृत स्थळ.',
      photoUrl: candidateData.photos?.[0] || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=400',
      photos: candidateData.photos && candidateData.photos.length > 0 ? candidateData.photos : ['https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=400'],
      aboutMe: candidateData.aboutMe || 'वंजारी समाज वर/वधू बायोडाटा नोंदणी.',
      isApproved: true,
      isVerified: true,
      aadhaarVerified: false,
      isFeatured: false,
      lastActive: 'आत्ताच (Just now)',
      membership: 'gold',
      createdAt: new Date().toISOString(),
  ...
  const saveBioDataSubmission = (
    data: Omit<BioDataSubmission, 'id' | 'createdAt' | 'status'>
  ) => {
    const newSubmission: BioDataSubmission = {
      ...data,
      id: 'bd-' + Date.now(),
      status: data.isSavedToPortal ? 'converted_to_member' : 'saved_only',
      createdAt: new Date().toISOString(),
    };

    setBioDataSubmissions((prev) => [newSubmission, ...prev]);
    syncDocToFirestore('biodataSubmissions', newSubmission.id, newSubmission);

    if (data.isSavedToPortal) {
      registerCandidateDirectly({
        ...
        occupation: data.jobTitle || data.businessTitle || '---',
        district: data.nativePlace || 'महाराष्ट्र',
        taluka: data.nativePlace || '',
        mobileNumber: data.mobile,
        whatsappNumber: data.whatsapp || data.mobile,
        photos: data.candidatePhotoUrl ? [data.candidatePhotoUrl] : [],
        aboutMe: `ऑनलाइन बायोडाटा मेकर द्वारे तयार केलेले प्रोफाइल. जन्मवेळ: ${data.birthTime || '---'}, गोत्र: ${data.gotra || '---'}`,
      });
    }

    logActivity('BioData Saved', `नवीन बायोडाटा नोंद प्राप्त झाली: ${data.fullName} (${data.mobile})`, data.fullName);
    addBroadcastNotification(`नवीन बायोडाटा अर्ज प्राप्त झाला: ${data.fullName}`, 'BioData Submission');
  };

  const convertBioDataToMember = (submissionId: string) => {
    const sub = bioDataSubmissions.find((s) => s.id === submissionId);
    if (!sub) return;
    ...
    registerCandidateDirectly({
      ...
      occupation: sub.jobTitle || sub.businessTitle || '---',
      district: sub.nativePlace || 'महाराष्ट्र',
      taluka: sub.nativePlace || '',
      mobileNumber: sub.mobile,
      whatsappNumber: sub.whatsapp || sub.mobile,
      photos: sub.candidatePhotoUrl ? [sub.candidatePhotoUrl] : [],
      aboutMe: `बायोडाटा अर्जावरून ॲडमिनने सदस्यात रूपांतरित केलेले प्रोफाइल. जन्मवेळ: ${sub.birthTime || '---'}, गोत्र: ${sub.gotra || '---'}`,
    });

    setBioDataSubmissions((prev) =>
      prev.map((s) => (s.id === submissionId ? { ...s, status: 'converted_to_member' } : s))
    );
    syncDocToFirestore('biodataSubmissions', submissionId, { status: 'converted_to_member' });
    logActivity('BioData Converted', `बायोडाटा अर्जाचे सदस्यात रूपांतर केले: ${sub.fullName}`, 'Admin');
  };

  const deleteBioDataSubmission = (submissionId: string) => {
    setBioDataSubmissions((prev) => prev.filter((s) => s.id !== submissionId));
    logActivity('BioData Deleted', `बायोडाटा नोंद डिलीट केली ID: ${submissionId}`, 'Admin');
  };
  ...
  // Truecaller & Mobile Number Verification Handlers
  const verifyPhoneNumber = async (
    profileId: string,
    method: 'truecaller' | 'otp' | 'admin',
    truecallerName?: string
  ): Promise<boolean> => {
    try {
      const timestamp = new Date().toISOString();
      const isTc = method === 'truecaller';
        ...
        prev.map(p => {
          if (p.id === profileId) {
            const updated: UserProfile = {
              ...
              truecallerVerified: isTc,
              truecallerName: truecallerName || p.fullName,
            };
            syncDocToFirestore('profiles', updated.id, updated);
            return updated;
          }
        ...
        setCurrentUser(prev =>
          prev
            ? {
                ...
                phoneVerificationMethod: method,
                truecallerVerified: isTc,
                truecallerName: truecallerName || prev.fullName,
              }
            : null
        );
      }
      ...
      logActivity(
        'phone_verified',
        `मोबाईल नंबर पडताळणी यशस्वी (${isTc ? 'Truecaller' : method.toUpperCase()}): ID ${profileId}`
      );
      return true;
    } catch (err) {
  ...
  const togglePhoneVerified = (profileId: string, status?: boolean) => {
    setProfiles(prev =>
      prev.map(p => {
        if (p.id === profileId) {
          const newStatus = status !== undefined ? status : !p.isPhoneVerified;
          const updated: UserProfile = {
            ...p,
            isPhoneVerified: newStatus,
            truecallerVerified: newStatus ? (p.truecallerVerified ?? true) : false,
            phoneVerifiedAt: newStatus ? new Date().toISOString() : undefined,
            phoneVerificationMethod: newStatus ? (p.phoneVerificationMethod || 'admin') : undefined
          };
          syncDocToFirestore('profiles', updated.id, updated);
          return updated;
        }
    ...
    if (currentUser && currentUser.id === profileId) {
      setCurrentUser(prev => {
        if (!prev) return null;
        const newStatus = status !== undefined ? status : !prev.isPhoneVerified;
        return {
          ...prev,
          isPhoneVerified: newStatus,
          truecallerVerified: newStatus ? (prev.truecallerVerified ?? true) : false,
          phoneVerifiedAt: newStatus ? new Date().toISOString() : undefined,
          phoneVerificationMethod: newStatus ? (prev.phoneVerificationMethod || 'admin') : undefined
        };
      });
    }

    logActivity('toggle_phone_verified', `ॲडमिनने मोबाईल पडताळणी स्थिती बदलली: ID ${profileId}`, 'Admin');
  };
    ...
    return [
      {
        ...
        targetProfileMobile: '+91 98221 00101',
  ...
  const updateAdminCredentials = async (
    ...
    let cleanUser = siteConfig.adminUsername || 'admin';
    let cleanPass = siteConfig.adminPin || '12345';
    let cleanName = siteConfig.adminCredentials?.name || 'मुख्य प्रशासक (Super Admin)';

    if (typeof credsOrUser === 'object' && credsOrUser !== null) {
      if (credsOrUser.username !== undefined) cleanUser = credsOrUser.username.trim();
      if (credsOrUser.password !== undefined) cleanPass = credsOrUser.password.trim();
      if (credsOrUser.displayName !== undefined || credsOrUser.name !== undefined) {
        cleanName = (credsOrUser.displayName || credsOrUser.name || '').trim();
      }
    } else if (typeof credsOrUser === 'string') {
      cleanUser = credsOrUser.trim();
      if (password !== undefined) cleanPass = password.trim();
      if (displayName !== undefined) cleanName = displayName.trim();
    }
    ...
    setSiteConfig((prev) => {
      ...
      try {
        localStorage.setItem('vanjari_jodi_site_config', JSON.stringify(updated));
        localStorage.setItem('vanjari_jodi_admin_password', cleanPass);
        localStorage.setItem('vanjari_jodi_admin_user', cleanUser);
      } catch (e) {}
      syncDocToFirestore('siteConfig', 'mainConfig', updated);
      return updated;
    });
    ...
    try {
      ...
      console.warn('Backend sync for admin credentials skipped:', e);
    }

    logActivity('Admin Password Updated', `मुख्य ॲडमिन पासवर्ड व क्रेडेंशियल्स अपडेट केले: ${cleanUser}`, 'Admin');
  };

  const loginAsGuest = (mobile?: string, name?: string, location?: string) => {
    const rawClean = (mobile || '').replace(/\D/g, '');
    alert(
      language === 'mr'
        ? '🔒 वधू-वरांच्या गोपनीयतेच्या सुरक्षेसाठी गेस्ट लॉगिन कायमचे बंद करण्यात आले आहे. कृपया आपल्या अधिकृत मोबाईल नंबरने लॉगिन किंवा मोफत नोंदणी करा.'
        : 'Guest login has been permanently disabled for member privacy. Please log in or register.'
    );
    ...
    return [
      {
        id: 'act-sample-1',
        userId: '1',
        userName: 'पूजा भास्कर सानप',
        userMobile: '+91 98221 00101',
        userType: 'registered',
        action: 'लॉगिन यशस्वी',
        details: 'पूजा सानप यांनी मोबाईल नंबर द्वारे सुरक्षित लॉगिन केले.',
        timestamp: new Date(Date.now() - 1000 * 60 * 5).toISOString(),
        category: 'login',
        deviceInfo: '📱 मोबाईल ॲप (Android)'
      },
      {
        id: 'act-sample-2',
        userId: '2',
        userName: 'अविनाश तात्यासाहेब मुंडे',
        userMobile: '+91 98220 11223',
        userType: 'registered',
        action: 'बायोडाटा पाहिला',
        details: 'अविनाश मुंडे यांनी पूजा भास्कर सानप यांचा बायोडाटा सविस्तर उघडून पाहिला.',
        timestamp: new Date(Date.now() - 1000 * 60 * 12).toISOString(),
        category: 'profile_view',
        targetProfileId: '1',
        targetProfileName: 'पूजा भास्कर सानप',
        deviceInfo: '📱 मोबाईल ब्राऊझर'
      },
      {
        id: 'act-sample-3',
        userId: '2',
        ...
        userMobile: '+91 98220 11223',
        userType: 'registered',
        action: 'पसंती (Like) नोंदवली',
        details: 'अविनाश मुंडे यांनी पूजा सानप यांच्या प्रोफाईलला लाईक पाठवले.',
        timestamp: new Date(Date.now() - 1000 * 60 * 14).toISOString(),
        category: 'like',
        targetProfileId: '1',
        targetProfileName: 'पूजा भास्कर सानप',
        deviceInfo: '📱 मोबाईल ॲप'
      },
      {
        id: 'act-sample-4',
        userId: '1',
        ...
        userMobile: '+91 98221 00101',
        userType: 'registered',
        action: 'Truecaller पडताळणी पूर्ण',
        details: 'पूजा सानप यांनी Truecaller द्वारे अधिकृत मोबाईल नंबर पडताळणी बॅज मिळवला.',
      ...
      },
      {
        id: 'act-sample-5',
        userId: '3',
        userName: 'प्राजक्ता अशोक नागरगोजे',
        userMobile: '+91 94220 33445',
        userType: 'registered',
        action: 'पत्रिका / ३६ गुण जुळवणी',
        details: 'प्राजक्ता नागरगोजे यांनी गुणमेलन कॅल्क्युलेटर द्वारे पत्रिका तपासली.',
        timestamp: new Date(Date.now() - 1000 * 60 * 40).toISOString(),
        category: 'biodata_kundali',
        deviceInfo: '💻 लॅपटॉप'
      },
      {
        id: 'act-sample-6',
        ...
        userMobile: '+91 98220 11223',
        userType: 'registered',
        action: 'ऑनलाईन पेमेंट पावती सादर केली',
        details: 'अविनाश मुंडे यांनी ₹1100 गोल्डन प्लॅनसाठी QR कोड पेमेंट करून UTR: 42398711092 सादर केली.',
        timestamp: new Date(Date.now() - 1000 * 60 * 55).toISOString(),
        category: 'payment',
        deviceInfo: '📱 Google Pay / PhonePe'
      },
      {
        id: 'act-sample-7',
        userId: 'guest-8821',
        userName: 'पाहुणे सदस्य (Guest)',
        userMobile: '+91 99000 00000',
        userType: 'guest',
        action: 'शोध फिल्टर्स वापरले',
        details: 'अतिथी सदस्याने बीड जिल्ह्यातील उच्चशिक्षित वर शोधले.',
  ...
  const trackUserAction = (
    ...
    const devInfo = typeof navigator !== 'undefined'
      ? (navigator.userAgent.includes('Mobile') ? '📱 मोबाईल ॲप / ब्राऊझर' : '💻 लॅपटॉप / कॉम्प्युटर')
      : '📱 मोबाईल ब्राऊझर';
    ...
    setUserActivityLogs(prev => [newLog, ...prev.slice(0, 499)]);
    syncDocToFirestore('userActivityLogs', newLog.id, newLog);
  ...
  const clearUserActivityLogs = () => {
    setUserActivityLogs([]);
    localStorage.removeItem('vanjari_jodi_user_activities');
    logActivity('क्लियर ॲक्टिव्हिटी लॉग्स', 'ॲडमिनने सर्व जुने युझर ॲक्टिव्हिटी लॉग्स रिसेट केले.', 'Admin');
  };
  ...
  const toggleBlockMemberAccess = (profileId: string) => {
    let targetName = profileId;
    let nextBlockedState = false;

    setProfiles((prev) =>
      prev.map((p) => {
        if (p.id === profileId) {
          nextBlockedState = !p.isBlocked;
          targetName = p.fullName || p.id;
          const updated = { ...p, isBlocked: nextBlockedState };
          syncDocToFirestore('profiles', updated.id, updated);
          return updated;
        }
        return p;
    ...
    logActivity(
      'Block Member Access',
      `ॲडमिनने ${targetName} सदस्याचा अक्सेस ${nextBlockedState ? 'पूर्णपणे ब्लॉक (Blocked)' : 'अन-ब्लॉक (Unblocked)'} केला.`,
      'Admin'
    );
    ...
    alert(
      language === 'mr'
        ? nextBlockedState
          ? `🚫 ${targetName} यांना यशस्वीरित्या ब्लॉक केले आहे! त्यांचे लॉगिन तात्काळ बंद झाले आहे.`
          : `✅ ${targetName} यांचे खाते अन-ब्लॉक (Unblocked) केले आहे! ते पुन्हा लॉगिन करू शकतात.`
        : nextBlockedState
          ? `🚫 ${targetName} has been blocked. Login access revoked.`
          : `✅ ${targetName} has been unblocked. Login access restored.`
    );
  };
  ...
  const [selectedKundaliCandidate, setSelectedKundaliCandidate] = useState<UserProfile | null>(null);

  const openKundaliMatching = (candidate?: UserProfile | null) => {
    if (candidate) {
      setSelectedKundaliCandidate(candidate);
    }
    setIsKundaliModalOpen(true);
  };

  // Single Kundli / Birth Chart Report Modal State
  const [isSingleKundliModalOpen, setIsSingleKundliModalOpen] = useState(false);

  const openSingleKundliGenerator = () => {
    setIsSingleKundliModalOpen(true);
  };

  // GitHub Official Code & Data Sync Center Modal State
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        setSearchFilters,
        resetFilters,
        filteredProfiles: sortedFilteredProfiles,
        viewedProfileIds,
        markProfileAsViewed,
        resetViewedProfiles,
        logout,
