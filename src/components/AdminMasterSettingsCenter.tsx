            ...
            <div className="lg:col-span-5 bg-gradient-to-br from-amber-50 via-rose-50/40 to-orange-50/50 rounded-2xl p-5 border-2 border-amber-300/80 flex flex-col items-center justify-center text-center space-y-3">
              ...
              <div className="w-full bg-white rounded-xl p-2.5 border border-amber-200 text-left space-y-1">
                ...
                <div className="text-[11px] font-black text-slate-800 flex justify-between">
                  ...
                  <span className="text-slate-500 truncate max-w-[150px]">{siteConfig.logoUrl || '/vanjari-jodi-official-logo-v3.png?v=3'}</span>
                </div>
              </div>
            </div>
              ...
              <div className="p-4 rounded-2xl bg-amber-50/60 border-2 border-amber-300 space-y-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        if (file) {
                          try {
                            ...
                            reader.onload = async (ev) => {
                              ...
                              if (base64) {
                                try {
                                  const res = await fetch('/api/admin/set-official-logo', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                  ...
                                  if (data.success && data.logoUrl) {
                                    ...
                                    localStorage.setItem('vanjari_jodi_active_logo', freshUrl);
                                    handleDirectFieldSave('logoUrl', freshUrl, 'नवीन गोल लोगो यशस्वीरित्या अपलोड व सिंक केला!');
                                  } else {
                                    localStorage.setItem('vanjari_jodi_active_logo', base64);
                                    handleDirectFieldSave('logoUrl', base64, 'नवीन गोल लोगो सेव्ह केला!');
                                  }
                                } catch {
                                  localStorage.setItem('vanjari_jodi_active_logo', base64);
                                  handleDirectFieldSave('logoUrl', base64, 'नवीन गोल लोगो सेव्ह केला!');
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      localStorage.removeItem('vanjari_jodi_active_logo');
                      handleDirectFieldSave('logoUrl', `/vanjari-jodi-official-logo.png?v=${Date.now()}`, 'मूळ अधिकृत लोगो रिस्टोअर केला!');
                    }}
                ...
                <div className="space-y-1 pt-1">
                  ...
                  <input
                    type="text"
                    defaultValue={siteConfig.logoUrl || '/vanjari-jodi-official-logo-v3.png?v=3'}
                    placeholder="उदा. https://... किंवा /vanjari-jodi-official-logo-v3.png?v=3"
                    onBlur={(e) => {
                      const val = e.target.value.trim();
