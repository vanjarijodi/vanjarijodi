...
interface AdminMemberQuickSettingsModalProps {
  ...
  onOpenFullEditModal?: (profile: UserProfile) => void;
}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-xl w-full shadow-2xl border-2 border-amber-300 overflow-hidden my-auto animate-fadeIn">
        ...
        {/* Modal Footer */}
        <div className="bg-slate-100 p-4 border-t border-amber-200 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
            ...
            {onOpenFullEditModal && (
              <button
                ...
                onClick={() => {
                  onClose();
                  onOpenFullEditModal(profile);
                }}
                className="px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-black rounded-xl text-xs transition cursor-pointer flex items-center gap-1.5 shadow-xs"
              >
                <Edit2 className="w-3.5 h-3.5" />
                <span>✏️ संपूर्ण प्रोफाईल एडिट करा</span>
              </button>
            )}
          </div>
          <button
