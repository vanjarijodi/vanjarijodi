  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-0 sm:p-4 animate-in fade-in duration-150 pt-safe pb-safe">
      <div className="bg-white w-full h-full sm:h-auto max-w-lg rounded-none sm:rounded-3xl shadow-2xl border-0 sm:border border-amber-200 overflow-hidden flex flex-col sm:my-auto max-h-none sm:max-h-[85vh]">
        ...
        {/* Push Notification & Sound Status Card */}
        <div className="px-3 py-2 bg-gradient-to-r from-amber-50 to-rose-50 border-b border-amber-200/80 flex items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-1.5 min-w-0">
            <span className="p-1 rounded-lg bg-amber-200/80 text-[#800C1E]">
              <Volume2 className="w-3.5 h-3.5" />
            </span>
            <div className="truncate">
              <span className="font-extrabold text-slate-800">सूचना ध्वनी: </span>
              <span className="text-emerald-700 font-bold">
                {permissionState === 'granted' ? 'सक्रिय (Active)' : 'ध्वनी तयार'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {permissionState !== 'granted' && (
              <button
                ...
                className="px-2 py-1 bg-[#800C1E] hover:bg-[#9E1428] text-white font-bold rounded-lg transition active:scale-95 text-[11px] cursor-pointer"
              >
                पुश चालू करा
              </button>
            )}
            <button
              ...
              className="px-2.5 py-1 bg-amber-400 hover:bg-amber-500 text-slate-950 font-black rounded-lg transition active:scale-95 flex items-center gap-1 text-[11px] cursor-pointer shadow-xs"
            >
              <Volume2 className="w-3 h-3" />
              <span>{isTestingSound ? 'वाजत आहे...' : 'ध्वनी चाचणी'}</span>
            </button>
          </div>
        </div>
