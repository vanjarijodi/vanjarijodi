...
export const AdminCustomPlanGrantModal: React.FC<AdminCustomPlanGrantModalProps> = ({
  ...
  const { updateProfileDirect, sendPushNotification, logActivity, plansList, addPaymentRequest } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Record transaction clearly as FREE / ADMIN GRANTED
    try {
      addPaymentRequest({
        userId: profile.id,
        ...
        amount: 0,
        paymentMethod: 'free_admin',
        utrNumber: `FREE-ADMIN-${Date.now().toString().slice(-6)}`,
        adminNote: `Payment Type: FREE / ADMIN GRANTED. Duration: ${calculatedExpiry.durationText}. Note: ${adminNote.trim()}`,
        planDurationText: calculatedExpiry.durationText,
        userPhotoUrl: profile.photoUrl || profile.photos?.[0] || '',
      } as any);
    } catch (e) {
      console.warn('Could not record free payment request:', e);
    }
