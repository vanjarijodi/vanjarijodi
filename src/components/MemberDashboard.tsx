...
import { calculatePlanExpiryInfo } from '../utils/planExpiryUtils';
import {
  User,
  Heart,
  Bell,
    ...
    <div className="min-h-screen bg-[#FFFDF5] text-slate-800 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white border-2 border-amber-300 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          {/* Compact Member Action Buttons Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 w-full pt-1">
            <button
              ...
              className="px-3 py-2 rounded-xl bg-gradient-to-r from-[#800C1E] to-[#A71930] hover:from-[#A71930] hover:to-[#800C1E] text-amber-100 text-xs font-black shadow-xs flex items-center justify-center gap-1 border border-amber-400/40 transition-all cursor-pointer active:scale-95"
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl bg-amber-100 hover:bg-amber-200 text-[#800C1E] text-xs font-black shadow-2xs flex items-center justify-center gap-1 border border-amber-300 transition-all cursor-pointer active:scale-95"
              title="माझा बायोडाटा पहा व डाऊनलोड करा"
            >
              <FileText className="w-3.5 h-3.5 text-[#800C1E] shrink-0" />
              <span>बायोडाटा</span>
            </button>
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black shadow-xs flex items-center justify-center gap-1 border border-emerald-400 transition-all cursor-pointer active:scale-95"
            >
            ...
            <button
              type="button"
              onClick={() => setIsEditProfileModalOpen(true)}
              className="px-3 py-2 rounded-xl bg-amber-400 hover:bg-amber-500 text-slate-950 text-xs font-black shadow-xs flex items-center justify-center gap-1 border border-amber-300 transition-all cursor-pointer active:scale-95"
            >
              <Edit className="w-3.5 h-3.5 text-slate-950 shrink-0" />
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-black shadow-2xs flex items-center justify-center gap-1 border border-rose-200 transition-all cursor-pointer active:scale-95"
              title="खात्यातून बाहेर पडा (लॉग आऊट)"
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-700 font-bold mt-0.5">
                    योग्य व सुशिक्षित स्थळांकडून उत्तम प्रतिसाद मिळण्यासाठी व बायोडाटा परिपूर्ण दिसण्यासाठी कृपया किमान ५ सुंदर फोटो नक्की अपलोड करा.
                  </p>
                </div>
              </div>
              <button
          ...
          <div className="flex flex-wrap items-center gap-1.5 pt-1 border-t border-amber-200/60">
            ...
            {!currentUser.isPhoneVerified ? (
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-extrabold flex items-center gap-1 shadow-2xs animate-pulse cursor-pointer"
                title="Truecaller द्वारे १ सेकंदात मोबाईल नंबर पडताळणी करा"
              >
                <Zap className="w-3 h-3 text-cyan-200 fill-current" />
                <span>Truecaller पडताळणी</span>
              </button>
            ) : (
              <span className="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-900 text-[11px] font-extrabold flex items-center gap-1 border border-blue-200">
            ...
            {!currentUser.isFaceVerified && (
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-[11px] font-extrabold flex items-center gap-1 shadow-2xs"
              >
                <ScanFace className="w-3 h-3 text-teal-200" />
                <span>चेहरा पडताळणी (Blue Tick)</span>
              </button>
            )}
            ...
            {!siteConfig?.hidePaymentDetailsGlobal && (
              <button
                type="button"
                onClick={() => setIsPaymentOpen(true)}
                className="px-2.5 py-1 rounded-lg bg-gradient-to-r from-[#A71930] to-[#C82333] text-amber-100 text-[11px] font-black flex items-center gap-1 shadow-2xs border border-amber-300/40"
              >
                <Crown className="w-3 h-3 fill-amber-300 text-amber-300" />
                <span>प्लॅन अपग्रेड</span>
              </button>
            )}
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-rose-50 text-slate-700 hover:text-rose-800 text-[11px] font-extrabold flex items-center gap-1 border border-slate-200 transition-all cursor-pointer ml-auto"
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-cyan-100 leading-relaxed max-w-2xl font-medium">
                  तुमचा मोबाईल नंबर <strong>{currentUser.mobile || ''}</strong> Truecaller किंवा OTP द्वारे सुरक्षितपणे पडताळणी करा. यामुळे तुमच्या प्रोफाइलवर अधिकृत <strong>"Truecaller व्हेरिफाइड"</strong> बॅज लागेल व इतर वधू-वरांचा विश्वास अधिक वाढेल.
                </p>
              </div>
            </div>
        ...
        {/* TAB 5: MEMBERSHIP */}
        {tab === 'membership' && (() => {
          const expiryInfo = calculatePlanExpiryInfo(currentUser, plansList);
          const activePlan = plansList.find((p) => p.id === currentUser.membership) || plansList[0];
          const isFreeOrAdmin = currentUser.membership === 'free' || currentUser.isFreeAdminPlan === true || currentUser.isFestiveFree === true;
          ...
          return (
            <div className="bg-white border-2 border-amber-200 rounded-3xl p-6 space-y-6 shadow-sm">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-amber-100 pb-4">
                <div>
                  <h3 className="text-lg font-black text-[#A71930] flex items-center gap-2">
                    ...
                    <span>माझी मेम्बरशिप व वैधता (Membership & Subscription)</span>
                  </h3>
                  <p className="text-xs text-slate-600 font-medium mt-0.5">
                    आपल्या विवाह सदस्यता प्लॅनची सविस्तर माहिती, वैधता व अनलॉक केलेले फायदे
                  </p>
                </div>

                <div className="flex items-center gap-2">
                  <span className={`px-3 py-1 rounded-xl text-xs font-black border flex items-center gap-1.5 ${
                    expiryInfo.statusBadgeColor === 'emerald'
                      ? 'bg-emerald-50 text-emerald-800 border-emerald-300'
                      : expiryInfo.statusBadgeColor === 'amber'
                      ? 'bg-amber-50 text-amber-900 border-amber-300 animate-pulse'
                      : expiryInfo.statusBadgeColor === 'rose'
                      ? 'bg-rose-50 text-rose-800 border-rose-300'
                      : 'bg-slate-100 text-slate-700 border-slate-300'
                  }`}>
                    <span className="w-2 h-2 rounded-full bg-current" />
                    <span>{expiryInfo.statusTextMr}</span>
                  </span>
                </div>
              </div>
              ...
              {expiryInfo.isExpiringSoon && (
                <div className="p-4 bg-gradient-to-r from-amber-500/20 via-amber-100 to-amber-500/20 border-2 border-amber-400 rounded-2xl flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <AlertCircle className="w-5 h-5 text-amber-800 shrink-0" />
                    <p className="text-xs font-bold text-amber-950">
                      ⚠️ आपली मेंबरशिप लवकरच संपणार आहे! अखंडित सेवा व संपर्क नंबर पाहण्यासाठी कृपया आजच रिन्यू करा.
                    </p>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="px-3.5 py-1.5 bg-[#800C1E] hover:bg-[#A71930] text-amber-100 rounded-xl text-xs font-black shrink-0 cursor-pointer shadow-xs"
                  >
                    आता रिन्यू करा
                  </button>
                </div>
              ...
              {/* Main Plan Card */}
              <div className="bg-gradient-to-br from-[#FFFDF5] to-amber-50/60 p-6 rounded-3xl border-2 border-amber-300 shadow-2xs space-y-5">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <span className="text-[11px] font-black text-amber-800 uppercase tracking-wider bg-amber-100 px-2.5 py-0.5 rounded-full border border-amber-300">
                      {isFreeOrAdmin ? '🎁 मोफत / ॲडमिन मंजूर प्लॅन' : '👑 सक्रिय प्रीमियम प्लॅन'}
                    </span>
                    <h4 className="text-xl font-black text-[#800C1E]">
                      {expiryInfo.planName}
                    </h4>
                    <p className="text-xs text-slate-600 font-medium">
                      {activePlan?.descriptionMr || 'वंजारी समाजातील अस्सल वधू-वर स्थळांचे संपर्क व बायोडाटा पाहण्याची संपूर्ण सुविधा.'}
                    </p>
                  </div>

                  <div className="text-right sm:border-l sm:border-amber-200 sm:pl-6">
                    <span className="text-[11px] text-slate-500 font-bold block">वैधता समाप्त तारीख</span>
                    <span className="text-base font-black text-slate-900 font-mono">
                      {expiryInfo.expiryDateFormatted}
                    </span>
                  </div>
                </div>

                {/* Progress Bar for Duration */}
                {!isFreeOrAdmin && expiryInfo.totalDays > 0 && expiryInfo.daysRemaining < 9000 && (
                  <div className="space-y-1.5 pt-2 border-t border-amber-200/80">
                    <div className="flex justify-between text-[11px] font-bold text-slate-600">
                      <span>वैधता प्रगती (Validity Usage)</span>
                      <span className="font-mono">{expiryInfo.daysRemaining} दिवस शिल्लक / एकूण {expiryInfo.totalDays} दिवस</span>
                    </div>
                    <div className="h-2.5 bg-amber-200/60 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${
                          expiryInfo.isExpired ? 'bg-rose-500' : expiryInfo.isExpiringSoon ? 'bg-amber-500' : 'bg-emerald-600'
                        }`}
                        style={{
                          width: `${Math.min(100, Math.max(5, (expiryInfo.daysRemaining / expiryInfo.totalDays) * 100))}%`,
                        }}
                      />
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3 border-t border-amber-200/80">
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>अमर्यादित बायोडाटा व फोटो पाहण्याची सोय</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>पालकांशी थेट संपर्क व मोबाईल नंबर अनलॉकिंग</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>३६ गुण कुंडली मिलन व पत्रिका जुळवणी</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>बायोडाटा PDF व JPG डाऊनलोड सुविधा</span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-amber-200/80">
                  <button
                    onClick={() => setIsPaymentOpen(true)}
                    className="px-5 py-2.5 bg-gradient-to-r from-[#800C1E] via-[#A71930] to-[#800C1E] hover:from-[#A71930] hover:to-[#800C1E] text-amber-100 font-black rounded-xl text-xs shadow-md transition flex items-center gap-2 cursor-pointer active:scale-95"
                  >
                    <Crown className="w-4 h-4 text-amber-300" />
                    <span>नवीन प्लॅन निवडा / अपग्रेड करा</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => {
                      window.dispatchEvent(new CustomEvent('vanjari_open_receipt_modal', {
                        detail: {
                          paymentId: currentUser.lastPaymentId || 'LIVE-PAY',
                          orderId: currentUser.lastOrderId || 'LIVE-ORD',
                          planName: expiryInfo.planName,
                          amount: activePlan?.price || 199,
                    ...
                    className="px-4 py-2.5 bg-white hover:bg-amber-50 text-slate-800 border border-amber-300 font-bold rounded-xl text-xs transition flex items-center gap-1.5 cursor-pointer shadow-2xs"
                  >
                    <FileText className="w-3.5 h-3.5 text-amber-700" />
                    <span>अधिकृत पावती (Official Receipt)</span>
                  </button>
                </div>
              </div>
            </div>
