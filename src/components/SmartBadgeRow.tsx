  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {isDoctor && (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-black bg-gradient-to-r from-teal-700 via-teal-800 to-emerald-900 text-teal-100 border border-teal-400 shadow-xs">
            <Stethoscope className="w-3.5 h-3.5 text-teal-300" />
            <span>
              {isGovt
                ? (isEn ? '🩺 Govt. Doctor / Medical Officer' : '🩺 शासकीय डॉक्टर / मेडिकल ऑफिसर')
                : (isEn ? '🩺 Doctor / Medical' : '🩺 डॉक्टर / मेडिकल ऑफिसर')}
            </span>
          </span>
        )}
        ...
        {additionalTags.map((tag, idx) => {
          ...
          if (
            (isGovt && (tagLower.includes('सरकारी') || tagLower.includes('शासकीय') || tagLower.includes('govt'))) ||
            (isDoctor && (tagLower.includes('डॉक्टर') || tagLower.includes('मेडिकल') || tagLower.includes('doctor') || tagLower.includes('वैद्यकीय'))) ||
            (isEngineer && (tagLower.includes('इंजिनिअर') || tagLower.includes('आयटी') || tagLower.includes('engineer'))) ||
            (isTeacher && (tagLower.includes('शिक्षक') || tagLower.includes('प्राध्यापक') || tagLower.includes('teacher'))) ||
            (isPolice && (tagLower.includes('पोलीस') || tagLower.includes('सैन्यदल') || tagLower.includes('police'))) ||
            (isOfficer && (tagLower.includes('वर्ग-१') || tagLower.includes('वर्ग-२') || tagLower.includes('अधिकारी') || tagLower.includes('officer')))
          ) {
            return null;
          }
          return (
