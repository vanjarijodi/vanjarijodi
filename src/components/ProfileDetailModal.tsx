...
export const ProfileDetailModal: React.FC<{
    ...
    try {
      ...
      if (side === 'front') {
        ...
        if (addNotification) {
          addNotification({
            type: 'success',
            title: 'आधार दस्तऐवज',
            message: 'आधार पुढचा फोटो (Front Side) यशस्वीरित्या सेव्ह झाला!',
          });
        }
      } else {
        ...
        if (addNotification) {
          addNotification({
            type: 'success',
            title: 'आधार दस्तऐवज',
            message: 'आधार पाठीमागचा फोटो (Back Side) यशस्वीरित्या सेव्ह झाला!',
          });
        }
      }
    } catch (err) {
      ...
      if (addNotification) {
        addNotification({
          type: 'error',
          title: 'त्रुटी',
          message: 'आधार दस्तऐवज अपलोड करताना त्रुटी आली.',
        });
      }
    } finally {
    ...
    try {
      ...
      if (result.success) {
        if (addNotification) {
          addNotification({
            type: 'success',
            title: 'फोटो जोडला',
            message: 'नवीन फोटो यशस्वीरित्या जोडला गेला!',
          });
        }
      } else {
        if (addNotification) {
          addNotification({
            type: 'error',
            title: 'त्रुटी',
            message: result.message || 'फोटो जोडता आला नाही.',
          });
        }
      }
    } catch (err) {
      ...
      if (addNotification) {
        addNotification({
          type: 'error',
          title: 'त्रुटी',
          message: 'फोटो अपलोड करताना त्रुटी आली.',
        });
      }
    } finally {
      ...
      if (navigator.share) {
        ...
        if (addNotification) {
          addNotification({
            type: 'success',
            title: 'कॉपी झाले',
            message: 'आपल्या बायोडाटाची माहिती कॉपी झाली आहे! आपण कोणालाही पाठवू शकता.',
          });
        }
      }
    }
  ...
  return (
    <>
        ...
        <div className="relative w-full min-h-full sm:min-h-0 sm:my-6 max-w-4xl mx-auto bg-[#FFFDF5] border-0 sm:border-2 border-amber-300 rounded-none sm:rounded-3xl shadow-2xl text-slate-800 flex flex-col">
          ...
          <div className="sticky top-0 z-40 flex items-center justify-between px-3.5 sm:px-6 py-2 sm:py-2.5 bg-amber-50/95 backdrop-blur-md border-b border-amber-200 text-slate-800 shadow-xs">
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
              ...
              {/* ADMIN FULL PROFILE EDIT BUTTON */}
              {isUserAdmin && (
                <button
                  type="button"
                  onClick={() => setIsAdminEditOpen(true)}
                  className="px-2.5 py-1 rounded-lg bg-gradient-to-r from-amber-600 to-[#A71930] hover:from-amber-700 hover:to-[#800C1E] text-amber-100 text-[11px] font-black flex items-center gap-1 border border-amber-400 shadow-xs cursor-pointer active:scale-95 transition"
                  title="✏️ या प्रोफाईलची सर्व माहिती एडिट करा (Admin Edit Profile)"
                >
                  <Edit3 className="w-3.5 h-3.5 text-amber-300" />
                  <span className="hidden sm:inline">✏️ प्रोफाईल एडिट करा</span>
                  <span className="sm:hidden">✏️ एडिट</span>
                </button>
              )}
          ...
          <div className="p-3.5 sm:p-6 space-y-5 flex-1">
            ...
            {isUserAdmin && adminViewMode === 'admin_controls' && (
              <div className="space-y-5 animate-fadeIn">
                ...
                {/* 🌟 PROMINENT ADMIN FULL PROFILE EDIT BANNER */}
                <div className="p-4 sm:p-5 bg-gradient-to-r from-amber-600 via-[#A71930] to-[#800C1E] text-white rounded-3xl border-2 border-amber-300 shadow-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <h4 className="text-sm sm:text-base font-black text-amber-200 flex items-center gap-2">
                      ...
                      <span>✏️ या सदस्याची संपूर्ण प्रोफाईल एडिट करा (Edit Entire Profile)</span>
                    </h4>
                    <p className="text-xs text-amber-100/90 font-medium">
                      नाव, वय, जन्मतारीख, कुंडली, शिक्षण, नोकरी, कुटुंब, फोटो, ओळखपत्र, संपर्क क्रमांक इत्यादी सर्व माहिती ॲडमिन म्हणून बदलू शकता.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setIsAdminEditOpen(true)}
                    className="px-5 py-2.5 bg-gradient-to-r from-amber-400 to-amber-300 hover:from-amber-300 hover:to-amber-200 text-slate-950 font-black rounded-2xl text-xs flex items-center gap-2 shadow-md transition cursor-pointer active:scale-95 shrink-0 border border-amber-200"
                  >
                    <Edit3 className="w-4 h-4 text-slate-950" />
                    <span>संपूर्ण प्रोफाईल एडिट करा</span>
                  </button>
                </div>
                ...
                <div className="p-4 sm:p-5 bg-white rounded-3xl border border-amber-300 shadow-sm space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                    ...
                    <div className="p-3.5 bg-slate-50 hover:bg-amber-50/50 rounded-2xl border border-slate-200 transition-colors flex flex-col justify-between gap-2.5">
                      <div>
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">ओळख पडताळणी बॅज</span>
                        <div className="mt-1 flex items-center gap-1.5">
                          ...
                          <span className={`text-xs font-black ${profile.aadhaarVerified ? 'text-blue-700' : 'text-slate-600'}`}>
                            {profile.aadhaarVerified ? 'ओळख प्रमाणित' : 'प्रलंबित'}
                          </span>
                        </div>
                      </div>
                      <button
              ...
              <div className="md:col-span-5 space-y-2">
                <div className="rounded-2xl overflow-hidden border border-amber-200/80 shadow-2xs bg-amber-50">
                  <InstagramPhotoCarousel
                    ...
                    aspectRatioClass="aspect-[4/5] min-h-[360px] sm:min-h-[440px] max-h-[560px]"
                    onPhotoClick={() => {
                      if (isPhotoBlurred) {
                        if (!currentUser || currentUser.isGuest) {
                          setLoginModalMode('member_otp');
                ...
                <div className="flex items-center justify-between text-xs font-semibold text-slate-600 px-1">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (isPhotoBlurred || photoAccess.isCompletelyHidden) {
                        if (!currentUser || currentUser.isGuest) {
                          setLoginModalMode('member_otp');
                          setIsLoginOpen(true);
                        } else {
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {(!currentUser || currentUser.isGuest) ? (
                        <span className="text-[10px] bg-amber-100 text-amber-900 border border-amber-300 font-bold px-2 py-0.5 rounded-full flex items-center gap-1 shadow-2xs">
                          <span>🔒</span>
                          <span>पूर्ण नाव पाहण्यासाठी नोंदणी / लॉगिन करा</span>
                        </span>
      ...
      {/* Admin Edit Profile Modal (Allows Admin to Edit Every Detail of This Profile) */}
      {isAdminEditOpen && (
