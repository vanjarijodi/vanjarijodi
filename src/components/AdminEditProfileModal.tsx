...
export const AdminEditProfileModal: React.FC<AdminEditProfileModalProps> = ({
    ...
    for (const file of filesToUpload) {
      try {
        ...
        if (res.success && res.url) {
          ...
          setPhotoError(res.errorMr || res.error || 'फोटो अपलोड करताना अडचण आली.');
        }
      } catch (err: any) {
        console.warn('Photo upload error:', err);
        setPhotoError('फोटो अपलोड करताना अडचण आली.');
      }
    }
  ...
  const handleDocUpload = async (e: React.ChangeEvent<HTMLInputElement>, targetDoc: 'aadhaar' | 'horoscope') => {
    ...
    try {
      const res = await uploadImageWithRetry(file, targetDoc === 'aadhaar' ? 'kyc' : 'general');
      if (res.success && res.url) {
        ...
        setDocError(res.errorMr || res.error || 'दस्तावेज अपलोड करताना त्रुटी आली.');
      }
    } catch {
      setDocError('दस्तावेज अपलोड करताना अडचण आली.');
    } finally {
