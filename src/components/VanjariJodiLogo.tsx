...
const FALLBACK_LOGO = '/vanjari-jodi-official-logo.png';
...
export const VanjariJodiLogo: React.FC<LogoProps> = ({
  ...
  const [currentLogoSrc, setCurrentLogoSrc] = React.useState(() => {
    return localStorage.getItem('vanjari_jodi_active_logo') || siteConfig?.logoUrl || '/vanjari-jodi-official-logo.png?v=20260926-user';
  });

  React.useEffect(() => {
    const active = localStorage.getItem('vanjari_jodi_active_logo');
    if (active) {
      setCurrentLogoSrc(active);
    } else if (siteConfig?.logoUrl) {
