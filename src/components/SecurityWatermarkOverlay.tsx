...
export const SecurityWatermarkOverlay: React.FC<SecurityWatermarkOverlayProps> = ({
  ...
  useEffect(() => {
    ...
    // Keydown listeners for printscreen / ctrl+p
  ...
  return (
    ...
    >
      ...
      {/* 1. Diagonal Tiled Text Across Container (Only for photos, not full modal text) */}
      {variant === 'photo' && (
        <div className="absolute inset-0 pointer-events-none z-30 overflow-hidden flex flex-col justify-around opacity-15 select-none rotate-[-18deg] scale-125">
          {[1, 2, 3, 4, 5].map((row) => (
            <div
              key={row}
