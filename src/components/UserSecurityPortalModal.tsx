...
export const UserSecurityPortalModal: React.FC<UserSecurityPortalModalProps> = ({ isOpen, onClose }) => {
  const { currentUser, language, setCurrentUser, updateUserPassword, setIsProfileRemovalModalOpen } = useApp();
          ...
          {activeTab === 'account_security' && (
            <div className="space-y-6">
              ...
              <div className="border border-gray-200 rounded-xl p-5 bg-white space-y-4">
                ...
                {/* Account / Profile Deletion Section */}
                <div className="mt-8 pt-6 border-t border-red-200">
                  <div className="flex items-center gap-2 mb-2 text-rose-700">
                    <Trash2 className="w-4 h-4" />
                    <h4 className="text-sm font-bold">
                      {language === 'mr' ? 'प्रोफाईल किंवा खाते कायमचे हटवणे (Delete Profile)' : 'Permanent Profile Deletion'}
                    </h4>
                  </div>
                  <p className="text-xs text-gray-600 mb-3">
                    {language === 'mr'
                      ? 'लग्न जुळल्यामुळे किंवा वैयक्तिक कारणास्तव आपली प्रोफाईल त्वरित हटवण्यासाठी खालील बटनावर क्लिक करा.'
                      : 'If your marriage is fixed or you wish to permanently remove your profile, click below.'}
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                      onClose();
                      setIsProfileRemovalModalOpen(true);
                    }}
                    className="px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-300 rounded-lg text-xs font-bold flex items-center gap-1.5 transition cursor-pointer"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>{language === 'mr' ? 'प्रोफाईल डिलीट करा (Delete Profile)' : 'Delete My Profile'}</span>
                  </button>
                </div>
              </div>
            </div>
