...
export const DynamicActionDock: React.FC<DynamicActionDockProps> = ({
  ...
  const handleOpenSearch = onOpenSearchModal || (() => setIsFilterOpen(true));
  const handleOpenRightFilter = onOpenRightFilter || (() => setIsRightDrawerOpen(true));
  ...
  const categories = [
    ...
    { id: 'verified', label: '🌟 पडताळणीकृत स्थळे', icon: ShieldCheck },
  ...
  // 1. HORIZONTAL CATEGORY CHIP-BAR (Only visible when viewing profiles)
  if (dockType === 'category_chip_bar') {
    if (currentView !== 'profiles') return null;
    return (
      <div className="w-full max-w-full overflow-hidden bg-white/95 backdrop-blur-md border-y border-amber-200 py-2 px-3 shadow-xs sticky top-[60px] z-30">
  ...
  if (dockType === 'floating_speed_dial') {
    return (
      <div className="fixed bottom-28 right-5 z-40 flex flex-col items-end gap-3 select-none">
        <AnimatePresence>
          {isSpeedDialOpen && (
            <motion.div
  ...
  // 4. INTERACTIVE BOTTOM SHEET (Positioned above mobile bottom nav bar)
  return (
    <div className="fixed bottom-16 md:bottom-0 inset-x-0 z-40 bg-white border-t-2 border-amber-300 shadow-2xl rounded-t-3xl transition-all">
      <div
      ...
      {isBottomSheetExpanded && (
        <div className="p-4 pt-1 max-w-4xl mx-auto border-t border-amber-100 grid grid-cols-2 sm:grid-cols-4 gap-2 pb-6">
