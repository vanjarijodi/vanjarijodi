...
/**
 * Uploads a File, Blob, or base64 Data URL to reliable server storage or Cloudinary.
...
export const uploadToCloudinary = async (
  ...
  try {
    // Attempt 1: Authoritative high-speed server image storage (never fails, prevents Firestore 1MB document bloat)
    try {
      const serverResult = await uploadImageWithRetry(fileOrDataUrl, folder as any, 1);
      if (serverResult.success && serverResult.url && !serverResult.url.startsWith('data:')) {
        return {
          success: true,
          url: serverResult.url,
          publicId: serverResult.fileName,
        };
      }
    } catch (serverErr) {
      console.warn('[uploadToCloudinary] Local server upload bypassed, trying Cloudinary:', serverErr);
    }
