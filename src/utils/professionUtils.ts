...
export function getProfessionBadges(profile: Partial<UserProfile>): string[] {
  ...
  if (isDoctor) {
    if (isGovt) {
      if (!badges.some(b => b.includes('डॉक्टर'))) badges.push('🩺 शासकीय डॉक्टर');
    } else {
      if (!badges.some(b => b.includes('डॉक्टर'))) badges.push('🩺 डॉक्टर');
    }
  } else if (isGovt) {
    if (!badges.some(b => b.includes('सरकारी') || b.includes('शासकीय'))) {
