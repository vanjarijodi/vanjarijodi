...
export async function downloadApkFile(
  ...
  // 4. Default to local server download route
  triggerLinkDownload('/downloads/VanjariJodi.apk', fileName);
}
...
export async function downloadAabFile(
  aabUrl: string = '/downloads/VanjariJodi-release.aab',
  appVersion: string = 'v2.5.0'
): Promise<void> {
  const cleanVersion = appVersion.replace(/[^a-zA-Z0-9.]/g, '') || 'v2.5.0';
  const fileName = `VanjariJodi_PlayStore_${cleanVersion}.aab`;
  triggerLinkDownload(aabUrl, fileName);
}

/**
 * Download Android Production Signing Keystore (.keystore)
 */
export async function downloadKeystoreFile(
  keystoreUrl: string = '/downloads/release.keystore'
): Promise<void> {
  triggerLinkDownload(keystoreUrl, 'release.keystore');
}

/**
 * Download Android Signing Key Credentials & Fingerprints (.txt)
 */
export async function downloadKeystoreInfo(
  infoUrl: string = '/downloads/KEYSTORE_INFO.txt'
): Promise<void> {
  triggerLinkDownload(infoUrl, 'KEYSTORE_INFO.txt');
}
